...
import { STRENGTH_FIELDS, DEFAULT_FORM, DEFAULT_LIFTS, STORAGE_KEYS } from "@/lib/defaults"
...
// progress moved to home page

export default function ProfilePage() {
  const router = useRouter()
  const { formData, setFormData, setPlan, setTrainingWeekdays, setChatMessages, setLifts } = useWorkout()
  ...
  // If the user hasn't created a profile yet, send them to onboarding.
  useEffect(() => {
      ...
      setChatMessages([
        {
          role: "assistant",
          content:
            "Plan regenerated. Want me to help you schedule it on your calendar?",
        },
      ])
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[var(--muted)]">Profile</p>
          <h1 className="mt-2 text-3xl font-black tracking-tight md:text-5xl">Your settings</h1>
          <p className="mt-3 max-w-prose text-base font-semibold text-[var(--muted)]">
            Your profile autosaves. You can edit it anytime — but you can’t create a “second profile” inside the app.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            variant="outline"
            className="rounded-full border-2 border-[var(--stroke)] font-black uppercase tracking-[0.12em] text-red-600"
            onClick={() => {
              if (!confirm("Delete saved profile? This will reset your profile data.")) return
              try {
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                {formData.units === "metric" ? (
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      else update({ weightKg: String(lbsToKg(Number(v))) })
                    }}
                    className="h-11 rounded-none border-2 border-[var(--stroke)] bg-[var(--surface)] font-black"
                    placeholder="e.g., 154"
                  />
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => update({ trainingDaysPerWeek: v })}
                >
                  <SelectTrigger className="h-11 rounded-none border-2 border-[var(--stroke)] bg-[var(--surface)] font-black">
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
